...
function PathBuilder(base = process.cwd(), {
  ...
  const opts = { JSON, path, fs, Promise, readdir_filter, filter, n, rel, load_only_traps, exclude_traps };
  if (!(this instanceof PathBuilder)) return new PathBuilder(base, { ...opts, n: 1 }, parts);
  this._properties = ['base', 'parts', 'JSON', 'path', 'fs', 'Promise', 'readdir_filter', 'rel', 'load_only_traps', 'exclude_traps'];
